...
class Template {
  ...
  public function make($content, $dados = null) {
	...
	public function makeSite($content, $dados = null) {
  	...
  	$dados['cat_url'] = $this->CI->uri->segment(2);	
  		$dados['origem'] = 'Site';
		$dados['header'] = $this->CI->load->view(THEME.'/template/v_header',$dados, true);
		$dados['content'] = $this->CI->load->view(THEME."/".$content ,@$dados, true);
		$this->CI->load->view(THEME.'/template/v_main',@$dados);
	}
